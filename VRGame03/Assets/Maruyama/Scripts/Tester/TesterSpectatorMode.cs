...
public class TesterSpectatorMode : MonoBehaviour
{
	bool inited = false;					//初期状態かどうか

	public Camera defaultExternalCamera;	//デフォルトカメラ
	OVRPlugin.Fovf defaultFov;
	public Transform SpectatorAnchor;		//ドローンカメラ
	public Transform Head;					//ヘッド
//#if OVR_ANDROID_MRC
	private OVRPlugin.Media.PlatformCameraMode camMode = OVRPlugin.Media.PlatformCameraMode.Disabled;	//カメラモード
	private bool readyToSwitch = false;	//レディースイッチ
	private Transform SpectatorCamera;	//ドローンカメラ

	// Dome sphere representation
	private float distance = 0.8f;	//距離
	private float elevation = 0.0f;	//高さ
	private float polar = 90.0f;	//ポーラー(極域)
	private const float distance_near = 0.5f;	//近い距離
	private const float distance_far = 1.2f;	//遠い距離
	private const float elevationLimit = 30.0f;	//ポーラー上限
//#endif
...
//#if OVR_ANDROID_MRC
		...
		SpectatorCamera = defaultExternalCamera.transform.parent;	//観客カメラをデフォルトカメラの親に設定
//#endif
	}
...
//#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN || OVR_ANDROID_MRC
		if (!defaultExternalCamera)	//デフォルトカメラが存在しないなら、警告
		{
...
//#endif
	...
	{
//#if UNITY_EDITOR_WIN || UNITY_STANDALONE_WIN || OVR_ANDROID_MRC
		if (inited)	//初期状態なら処理をしない
			return;

//#if OVR_ANDROID_MRC
		if (!OVRPlugin.Media.GetInitialized())	//初期化されていないなら処理を飛ばす
			return;
//#else
		if (!OVRPlugin.IsMixedRealityInitialized())	//複合現実が初期化されていないなら、処理をしない
			return;
//#endif

		OVRPlugin.ResetDefaultExternalCamera();	//デフォルトのExternalCameraをリセット
		Debug.LogFormat("GetExternalCameraCount before adding manual external camera {0}", OVRPlugin.GetExternalCameraCount());
		UpdateDefaultExternalCamera();	//デフォルトのExternalCameraをアップデータ
