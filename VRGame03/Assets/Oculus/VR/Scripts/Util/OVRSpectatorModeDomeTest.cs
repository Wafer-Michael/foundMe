...
public class OVRSpectatorModeDomeTest : MonoBehaviour {
	...
	[SerializeField]
	private UnityEngine.TextMesh m_debugText;
	...
	{
        if (OVRInput.GetDown(OVRInput.RawButton.X))	//Xを押したら
        {
			m_debugText.text = "";	//textのリセット
        }

		//m_debugText.text += "Initialize" + "\n";
		//m_debugText.text += "InputTrigger" + "\n";

		//OVRPlugin.Media.GetPlatformCameraMode().ToString();

		//string modeString = OVRPlugin.Media.GetPlatformCameraMode().ToString();
		//var mode = OVRPlugin.Media.PlatformCameraMode.SpectatorMode;
		...
		if (OVRInput.GetDown(OVRInput.Button.PrimaryIndexTrigger) || OVRInput.GetDown(OVRInput.Button.SecondaryIndexTrigger))
		{
			...
			if (camMode == OVRPlugin.Media.PlatformCameraMode.Disabled && readyToSwitch)
			{
				...
				bool isSet = OVRPlugin.Media.SetPlatformCameraMode(OVRPlugin.Media.PlatformCameraMode.Initialized);
				//bool isSet = OVRPlugin.Media.SetPlatformCameraMode(OVRPlugin.Media.PlatformCameraMode.SpectatorMode);	//オリジナル
				m_debugText.text += isSet.ToString() + "\n";
				
				StartCoroutine(TimerCoroutine());
				camMode = OVRPlugin.Media.GetPlatformCameraMode();

				m_debugText.text += "SpectatorMode" + "\n";	//オリジナル記載
				//m_debugText.text += OVRPlugin.Media.GetPlatformCameraMode().ToString(); + "\n";	//オリジナル記載
				string modeString = OVRPlugin.Media.GetPlatformCameraMode().ToString();
				m_debugText.text += modeString + "\n";
			}
